...
Engine 1.1.0 includes the fix for Codex AgentMessage content blocks with type Text and provider-neutral transcript adapters. The regression fixture preserves the observed event shape with synthetic message text.
...
The public installer and source package are tested separately with temporary fake integration directories and no model calls. Existing vaults can be connected through the integration-only path, which is covered by the same reversible-file logic. See tests/test_public_install.py. Run all tests with python -m unittest discover -s tests -v.
